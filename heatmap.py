...
def HEATMAP(df, prefix, scale="row", metric="euclidean", method="single",
              width=400, height=400, palette="Spectral_r"):
    df.index.name = "row"
    zscore = Zscor(df, scale=scale)
    ...
    output_file("{0}.HEATMAP.html".format(prefix),
...
def main():
    ...
    HEATMAP(df, prefix=prefix, scale=args.scale,
